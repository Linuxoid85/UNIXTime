...
Эта статья является продолжением цикла обзоров различных «экзотических» ОС. [Первая статья](https://linuxoid85.github.io/UNIXTime/articles/os/serenity/) была посвящена Unix-like ОС, разиваемой с нуля.
...
В меню мыстрых настроек есть «Shortcuts», смысла и основного назначения которых я так и не понял. Может быть они служат для того, чтобы закреплять какие-то часто используемые приложения и сайты для быстрого к ним доступа, но у меня они так и не заработали. Нажимаешь на них и не видишь ровным счётом никакой реакции.
...
Мне пришлось доскрочно завершить знакомство с этой «ОС», поскольку чем дольшще я её использовал, тем чаще у меня возникали мысли о том, что это и не ОС вовсе, а какой-то макет. В действительности, интерфейс у dahlia симпатичный, но куча мелких косяков, куча нереализованных или просто не работающих функций сводят все её плюсы (коих крайне немного) на нет. Кроме того, я так понимаю, что эта система какое-то время уже не развивается, если последний готовый образ датируется 2022 годом... Да и что за задумка у этой системы? Быть «заменителем» ChromeOS? А кому нужна эта кастрированная ОС от Google? Я не знаю ни одного человека, который пользуется ею на постоянной основе. Я не видел ни одного пользователя Chromebook. Я не видел, чтобы сейчас продавались эти хромбуки. Так зачем нужна dahliaOS, да ещё и на очень ранней стадии развития? Непонятно. Есть проекты и по круче и куда функциональнее, чем dahlia, ну например та же самая SerenityOS, которая вообще написана с нуля. Вот от неё есть хоть какая-то польза, ведь как минимум она приносит удовольствие её разработчикам, а как максимум даёт им ценный опыт не только программирования, но и разработки ПО различного типа, начиная от каких-то десктопных «Карт», заканчивая реализацией многозадачности в ядре ОС.
