...
<small>Скриншот взят с <a href="https://serenityos.org/">сайта</a> проекта. Вы можете увидеть больше <i>авторских</i> скриншотов в подразделе "<a href="./gallery/">Галерея</a>"</small>
...
Производительность после первого запуска просто ужасная. Забегая вперёд отмечу, что после второго всё стало нормально.
...
Что касается доступного ПО, то здесь не всё так гладко. Пакетных менеджеров и прочих привычных нам инструментов для управления тут нет, но разработчики заявляют о наличии системы портов. И вообще говорят, что управление ПО здесь было реализовано под влиянием систем BSD. Что ж, если там действительно есть что-то от BSD, то я очень рад — уж это куда лучше того кошмара, что есть в Linux...

В файле `/usr/Ports/AvailablePorts.md` представлена таблица с доступным ПО. Этот файл весит 78,4 Кб. Но всё равно нужного именно *пользователю* ПО там не так много. Например, нормальных браузеров там так и не ждите. Зато есть всякие разные инструменты для разработки вроде кучи всяких библиотек, систем сборки и прочего. В любом случае я не думаю, что SerenityOS подойдёт для обычного пользователя ввиду отсутствия ПО. И я не думаю, что в будущем ситуация будет лучше: современное ПО достаточно сложно и громоздко, чтобы портировать его на другие ОС. Кому это сейчас надо? Даже в какой-то условной FreeBSD, пользователей и контрибьюторов у которой куда больше, существует большая проблема с доступностью того или иного ПО. Что уж говорить про обзираемую ОС?

## Подводя итог

Что можно сказать об этой ОС? Несмотря на то, что система не задумывалась как что-то серьёзное, получилась действительно неплохая вещь с продвинутым (хоть и очень олдскульным) графическим интерфейсом, своей реализацией всего, что в ней есть после сборки, да ещё и на языке C++, а не на моднейших Rust и прочих, в значительной степени упрощающих разработку подобных вещей. Стоит отдать должное разработчикам: помимо самОй ОС они работают ещё и над свом браузером LadyBird, который не так давно отпочковался от основного проекта и ушёл в свободное плавание.

Кому она подойдёт? Наверное тем, кто интересуется строением и разработкой операционных систем: участие в её разработке значительно повысит умения как программиста, да ещё и даст опыт в разработке подобных ОС. Подойдёт и энтузиастам, которые просто хотят посмотреть на то, что есть и такие творения. На этом, на мой взгляд, всё. Обычному пользователю тут явно делать нечего, поскольку здесь нет практически никакого пользовательского ПО, да и поддержка железа не настолько развитая, чтобы эта ОС вышла куда-то [за пределы](https://github.com/SerenityOS/serenity/blob/master/Documentation/BareMetalInstallation.md) виртуальной машины QEMU. К тому же SerenityOS — наглядный пример «вещи в себе». Тут тебе и своё ядро, и своя стандартная библиотека С++, свой GUI, написанный с применением своего фреймворка LibGUI...

Изменится ли что-то в будущем? Однозначно да. Но не так, чтобы это было видно. На примере проектов React OS и Haiku мы видим, что эти долгострои разрабатываются многие годы, но так и не становятся популярными из-за того, что нужны разве что таким же энтузиастам, как и их разработчики, а поэтому в них нет ни необходимых драйверов под современное востребованное железо, ни какого-либо сложного софта. Да и где гарантии того, что завтра над этим проектом продолжат работать? Конечно, SerenityOS — это исключительно любительский проект, поэтому требования к нему с моей стороны достаточно завышены. Эта ОС существует и даже как-то развивается. А для чего она нужна? Для развлечения. Чего же ещё надо?
