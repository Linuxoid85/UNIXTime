...
Каталог `/proc` содержит информацию о процессах. Более ничего. В том же Linux этот каталог содержит больше данных, например `/proc/uname` информацию о ядре, `/proc/meminfo`, `/proc/cpuinfo` и т.д.
